...
- LLM: `llm_A_001`(generic baseline) / `llm_ux_A_001`(uxagent) 처럼 조건·agent_arch 식별 가능하게 부여
...
## 3. 실제 LLM 시뮬레이션 실행 절차
OpenAI 호환 백엔드를 `OPENAI_BASE_URL` 로 붙입니다. 본 연구는 **로컬 Ollama `qwen2.5:32b`**(quota 무관)로 수집합니다. agent 아키텍처 상세는 [docs/UXAGENT.md](UXAGENT.md).
```bash
npx next build && npx next start -p 3000 &       # 서버(API 라우트 포함; 코드 변경 후 반드시 재빌드)
set -a; . ./.env.local; set +a                   # Ollama 엔드포인트/모델 주입
npx tsx simulation/generate-personas.ts --count=20                                  # uxagent: persona 20명 1회 생성
AGENT_ARCH=uxagent npx tsx simulation/run.ts --variant=A --persona-id=p01 --participant-id=llm_ux_A_001 --headless=true
npx tsx simulation/run.ts --variant=A --participant-id=llm_A_001 --headless=true     # generic baseline(persona 없음)
```
- **agent_arch**: `uxagent`(persona+기억+반성+인터뷰 설문, 주 데이터) | `generic`(상태→action 단일 호출, baseline). provenance·분석에서 분리.
- **provider**: `LLM_PROVIDER`(미지정 시 키 자동감지, 둘 다 없으면 `mock`). 키가 없으면 mock 으로 동작(실험 데이터 아님).
- **mock 자동 fallback 안 함**: 실제 LLM 실패 시 mock 으로 섞지 않고 run 중단(clean 보존). 개발용만 `LLM_ALLOW_MOCK_FALLBACK=true`.
- 연결 전 확인: **Provider/모델 설정**, **재현성(action `LLM_TEMPERATURE=0.2` 고정; 변동은 persona)**, **로그에 모델·temperature·agent_arch·persona_id 기록 여부**.
- LLM 세션도 `participant_type=llm` + 식별용 `participant_id`(예: `llm_ux_A_001`)를 부여해 human 과 구분. uxagent run 은 `logs/trace_{sid}.json`(think-aloud) 저장.
...
```
- A/B 는 `ui_variant`, human↔LLM 은 `participant_type`, LLM 의 generic baseline↔uxagent 는 `agent_arch` 컬럼으로 그룹핑.
